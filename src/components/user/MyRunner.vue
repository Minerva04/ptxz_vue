<template>
    <div class="table-container">
        ...
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>我的跑腿订单</el-breadcrumb-item>
        </el-breadcrumb>
        ...
        <el-select v-model="queryInfo.status" placeholder="选择订单状态" style="width: 200px; margin-left: 20px;">
            ...
            <el-option label="未被接受" value="0"></el-option>
            <el-option label="已被接受" value="1"></el-option>
            <el-option label="对方发起仲裁" value="2"></el-option>
            <el-option label="我方发起仲裁" value="3"></el-option>
                ...
                <el-table-column label="操作" align="center">
                    <template slot-scope="scope">
                        <el-button v-if="scope.row.status === 0" type="primary" size="mini"
                            @click="initRunner(scope.row.id)" round>修改订单信息</el-button>

                        <el-dialog title="修改订单信息" :visible.sync="changeInfo">
                            <el-form :model="runner" ref='goodsForm' label-width="120px" class="form-container"
                                :rules="rules">
                                <el-form-item label="类型描述" prop="type">
                                    <el-input v-model="runner.type" clearable></el-input>
                                </el-form-item>
                                <el-form-item label="详细信息" prop="description">
                                    <el-input type="textarea" v-model="runner.description"></el-input>
                                </el-form-item>
                                <el-form-item label="联系方式" prop="contact">
                                    <el-input v-model="runner.contact"></el-input>
                                </el-form-item>
                            </el-form>
                            <div slot="footer" class="dialog-footer">
                                <el-button @click="changeInfo = false">取 消</el-button>
                                <el-button type="primary" @click="change(checkId)">修 改</el-button>
                            </div>
                        </el-dialog>
                        ...
                        <el-button  v-if="scope.row.status ==1||scope.row.status ==2||scope.row.status ==3" type="success" size="mini" @click="confirmInfo=true;this.checkId=scope.row.id" round>确认订单</el-button>
                        <el-dialog title="确认订单" :visible.sync="confirmInfo" width="30%">
                            <span>是否确认当前订单？</span>
                            <span slot="footer" class="dialog-footer">
                                <el-button @click="confirmInfo = false">取 消</el-button>
                                <el-button type="primary" @click="confirm(checkId)">确 定</el-button>
                            </span>
                        </el-dialog>
...
export default {
    mounted() {
        if (window.sessionStorage.getItem('myRunnerNum')) {
            this.queryInfo.pageNum = window.sessionStorage.getItem('myRunnerNum');

        }
        if (window.sessionStorage.getItem('myRunnerSize')) {
            this.queryInfo.pageSize = window.sessionStorage.getItem('myRunnerSize');
        }
        ...
        return {
            ...
            rules: {
                type: [
                    ...
                    { max: 10, min: 1, message: "长度在10个字符以内", trigger: "blur" },
                ],
                description: [
                    { required: true, message: "请输入订单详细信息", trigger: "blur" },
    ...
    methods: {
        ...
        confirmCancel(id) {
            ...
            this.$http.get(`runner/publisherconfirmcancel/${id}`).then((res) => {
                if (res.data.code == 1) {
        ...
        cancel(id) {
            ...
            this.$http.get(`runner/publishercancel/${id}`).then((res) => {
                if (res.data.code == 1) {
                    this.$message.success(res.data.data);
                    this.getMyRunnerList();
                } else if (res.data.code == 2) {
        ...
        confirm(id){
            ...
            this.$http.get(`runner/publishconfirm/${id}`).then((res) => {
                if (res.data.code == 1) {
                    this.$message.success(res.data.data);
                    this.getMyRunnerList();
                } else if (res.data.code == 2) {
        ...
        deleteGoods(id) {
            ...
            this.$http.delete(`runner/${id}`).then((res) => {
                if (res.data.code == 1) {
                    this.$message.success(res.data.data);
                    this.getMyRunnerList();
                } else if (res.data.code == 2) {
        ...
        getMyRunnerList() {
            this.$http.get(`runner/myrunner`, { params: { pageNum: this.queryInfo.pageNum, pageSize: this.queryInfo.pageSize, name: this.queryInfo.name, status: this.queryInfo.status } }).then((res) => {
                if (res.data.code == 1) {
        ...
        change(id) {
            this.$refs.goodsForm.validate(valid => {
                if (valid) {
                    this.$http.put(`runner/change/${id}`, this.runner).then((res) => {
                        if (res.data.code == 1) {
        ...
        putJudging(id) {
            this.judgingInfo = false,
                this.$http.put(`/judging/publisher/${id}`, this.judgingForm).then((res) => {
                    if (res.data.code == 1) {
                        this.$message.success(res.data.data);
                    } else if (res.data.code == 2) {
                        this.$message.error(res.data.msg);
