<template>
  ...
  <div>
    ...
    <el-breadcrumb-item :to="{ path: '/mydetail' }">我的信息</el-breadcrumb-item>
    ...
    <el-form :model="form" ref="form" label-width="80px" class="user-form">
      <div class="avatar-container">
        ...
        <el-form-item label="修改头像" prop="file">
          <el-upload class="upload-demo" action="api/file/upload" :on-success="handleUploadSuccess"
...
export default {
  mounted() {
    this.$http.get(`user`).then((resp) => {
      if (resp.data.code == 1) {
        this.img = `http://localhost:7070/api/file/download?name=${resp.data.data.head}`
